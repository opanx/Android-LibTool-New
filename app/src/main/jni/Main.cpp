                ...
                if (ImGui::BeginCombo("Game##preset", preview))
                {
                    for (int i = 0; i < numGamePresets; i++)
                    {
                        ...
                        if (ImGui::Selectable(presets[i].name, selected))
                        {
                            ...
                            ConfigSet("selectedGamePreset", i);
                        }
                        if (selected)
                        {
                            ImGui::SetItemDefaultFocus();
...
void on_init()
{
    ...
    initialStyle = ImGui::GetStyle();
    ConfigInit();
    // Load game preset BEFORE waiting for library
    ...
    LOGI("Target lib: %s (preset: %s)", getTargetLibName(), gamePresets[selectedGamePreset].displayName);
